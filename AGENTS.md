...
1. **Nothing in a commit message is ever computed by the shell.** No backticks, no `$(...)`, no `$VAR`, no unquoted heredoc, anywhere in the command that produces the message or the file it is read from. A commit message is literal text you wrote, and only that.
...
5. **Read every commit message back before you push. All of them, every time.** Run `git log origin/main..HEAD --format='--- %h%n%B'` and read the output top to bottom. "Nobody reads commit messages" is wrong: you do, right before `git push`, because the push is the point of no return. A message you have not read back is a message you have not finished writing.
