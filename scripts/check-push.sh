...
# messages themselves, in order, oldest first.
#
# Usage: invoked by git as the pre-push hook. Arguments: remote name, remote
# URL. Stdin: one line per ref being pushed —
...
if [ "$failed" -ne 0 ]; then
  ...
  echo "   Fix the commit locally (amend or rewrite), then push again. Do not --no-verify"
