...
| `thread-viewing`    | Reading one thread as a conversation: who wrote what, day and unread dividers, grouping, older history, folding long bodies. |
...
- **React is allowed, and nothing else.** A story may export hooks (for example
  `useThreadHistory`), because every client is React. React is an optional peer
  dependency, so a client brings its own copy. A client whose bundler
