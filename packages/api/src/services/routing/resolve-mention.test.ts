...
describe('resolveAgentFromMention', () => {
  ...
  it('matches text with agent name (identity name, not agent_id)', async () => {
    ...
    expect(result).toEqual({ sbSlug: 'custom-agent', sbId: 'id-custom' });
  });
});
