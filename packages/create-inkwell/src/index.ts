...
/**
 * create-inkwell — Zero-friction Inkwell setup wizard.
 *
 * Orchestrates: clone → install → database → server → auth → init → memory → awaken
 *
 * Every step is idempotent. If setup fails midway, re-run `npx @inklabs/create-inkwell`
...
function showBanner(): void {
  console.log();
  console.log(chalk.bold('  create-inkwell'));
...
async function cloneRepo(state: ProgressState): Promise<boolean> {
  ...
  const spinner = ora('  Cloning Inkwell repository...').start();
  try {
...
async function startServer(state: ProgressState): Promise<boolean> {
  ...
  console.log(chalk.dim('    Starting Inkwell server + web dashboard in the background...'));
...
async function initInk(state: ProgressState): Promise<boolean> {
  stepHeader(7, 'Initialize Inkwell');
  ...
  if (code !== 0) {
    fail('Inkwell initialization failed');
    return false;
  }

  console.log();
  ok('Inkwell initialized');
  return true;
}
...
async function setupMemory(state: ProgressState): Promise<boolean> {
  ...
  console.log(
    chalk.dim('    Inkwell works without this — you can always run "sb memory install" later.')
  );
  console.log();
...
function showComplete(state: ProgressState): void {
  console.log();
  console.log(chalk.bold.green('  ✓ Inkwell is set up and running!'));
  console.log();
...
const STEPS: StepDef[] = [
  ...
  { id: 'init', fn: initInk, required: true },
...
function showHelp(): void {
  console.log(`
  ${chalk.bold('create-inkwell')} v${getVersion()}

  ${chalk.dim('Set up Inkwell — AI beings with identity, memory, and coordination.')}

  ${chalk.bold('Usage')}
    npx @inklabs/create-inkwell [target-directory]
  ...
  ${chalk.bold('Examples')}
    npx @inklabs/create-inkwell                         Interactive setup (prompts for directory)
    npx @inklabs/create-inkwell my-project              Clone into ./my-project
    npx @inklabs/create-inkwell ~/dev/inkwell               Use an absolute path
...
async function main(): Promise<void> {
  ...
  if (!targetDir) {
    targetDir = await input({
      message: 'Where should we set up Inkwell?',
      default: DEFAULT_DIR,
    });
  }
