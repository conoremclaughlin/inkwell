-- Generation identity at the DB boundary (Lumen, PR #563 round 20).
--
-- Two flaws in the timestamp-only tombstone:
--   * the reclaim predicate allowed `cli_turn_stopped_at <= p_not_stopped_after`
--     — EQUALITY claimed, so a tombstone scoped to a marker's own birth time
--     did not fence that marker's parked claim;
--   * a timestamp cannot distinguish same-millisecond attempts by two
--     coexisting wrapper generations on one session.
--
-- The fence is now two INDEPENDENT mechanisms:
--   1. `cli_turn_stopped_at` — stamped only by REAL fenced stops (the turn
--      provably ended). A reclaim passes only when its marker is STRICTLY
--      newer (`cli_turn_stopped_at < p_not_stopped_after`).
--   2. `cli_turn_fence_generation` — stamped by wrapper scope-ends and
--      record-lost stops with the SENDER'S generation. A reclaim carrying
--      `p_wrapper_generation` is refused iff it matches — exactly the stale
--      generation's parked claims, with no timestamp involvement, so a
--      successor generation's claims pass at any clock resolution.
-- Generations are UUIDs and never reused, so the scalar needs no clearing.

ALTER TABLE public.sessions ADD COLUMN IF NOT EXISTS cli_turn_fence_generation text;
...
CREATE OR REPLACE FUNCTION public.claim_turn_epoch(
  ...
  p_wrapper_generation text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
...
BEGIN
  ...
  WHERE id = p_session_id
    ...
    AND (
      p_wrapper_generation IS NULL
      OR cli_turn_fence_generation IS NULL
      OR cli_turn_fence_generation <> p_wrapper_generation
    )
...
DROP FUNCTION IF EXISTS public.claim_turn_epoch(uuid, boolean, timestamptz, uuid, jsonb);
